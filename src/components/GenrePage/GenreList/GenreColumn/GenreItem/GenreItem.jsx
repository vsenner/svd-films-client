  ...
  return (
    <div className="genre-list__item">
      <label className='genre-list__label'>
        <input id={genre.id} type="radio" className='genre-list__yes' name={genre.id} defaultChecked={yes} onClick={() => {
          if(yes){
      ...
      <label className='genre-list__label'>
        <input type="radio" id={genre.id + 1} className='genre-list__no' name={genre.id} defaultChecked={no} onClick={() => {
          if(no){
            unselectToggle(genre.id)
          }else{
            changeGenres(no)
